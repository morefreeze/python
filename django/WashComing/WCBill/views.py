...
def submit(request):
    if request.method == 'GET':
        ...
        if fo_bill.is_valid():
            ...
            mo_bill.get_time = d_data.get('get_time')
            mo_bill.return_time = d_data.get('return_time')
            mo_bill.own = mo_user
            mo_bill.lg = None
...
# query all include deleted
                l_bills = Bill.objects.filter(own=mo_user)\
                    .order_by('get_time', 'return_time', '-bid')[i_offset:i_offset+i_limit]
            else:
                l_bills = Bill.objects.filter(own=mo_user,deleted=i_deleted)\
                    .order_by('get_time', 'return_time', '-bid')[i_offset:i_offset+i_limit]
            d_response = dict()
            d_response['errno'] = 0
            d_response['data'] = []
            for mo_bill in l_bills:
